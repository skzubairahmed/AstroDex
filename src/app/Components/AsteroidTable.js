...
export default function PaginatedAsteroidTable({ asteroids, itemsPerPage }) {
    ...
    const filteredAsteroids = asteroids.filter((item) => {
        if (filters === "pha") return item.pha === "Y";
        return true;
    });

    let totalPages = Math.ceil(filteredAsteroids.length / itemsPerPage) || 1;
    ...
    const currentData = filteredAsteroids.slice(startIndex, startIndex + itemsPerPage);
    ...
    return (
        <div className="w-full h-fit overflow-x-auto mx-auto p-3 text-text-primary bg-card-surface rounded-lg border border-satellite/20">
            <select 
                id="filter" 
                value={filters}
                className="flex border border-satellite p-2 rounded-md bg-transparent mb-4" 
            ...
            <table className="text-left w-full border-collapse text-sm">
                ...
                <tbody className="divide-y divide-satellite/20 text-secondary/20">
                    {currentData.length > 0 ? (
                                ...
                                <tr key={item.spkid} className="hover:bg-satellite/40 transition">
                                    ...
                                    <td className="py-3 px-3">
                                        <button className="px-2 py-2 bg-satellite/30 border border-satellite rounded-lg" onClick={() => toggleBookmark(item)}>
                                            {isBookmarked ? (
                                                <span><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#F3F4F6"><path d="M713-600 600-713l56-57 57 57 141-142 57 57-198 198ZM200-120v-640q0-33 23.5-56.5T280-840h240v80H280v518l200-86 200 86v-278h80v400L480-240 200-120Zm80-640h240-240Z"/></svg></span>
                                            ) : (
                                                <span><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#F3F4F6"><path d="M200-120v-640q0-33 23.5-56.5T280-840h240v80H280v518l200-86 200 86v-278h80v400L480-240 200-120Zm80-640h240-240Zm400 160v-80h-80v-80h80v-80h80v80h80v80h-80v80h-80Z"/></svg></span>
                                            )}
                                        </button>
                                    </td>
                                    <td className="py-3 px-3">
                                        <button className="px-2 py-2 bg-satellite/30 border border-satellite rounded-lg text-white" onClick={() => router.push(`/asteroids/${item.spkid}`)}>
                                            View
                                        </button>
                                    </td>
                                </tr>
                        ...
                        <tr>
                            <td colSpan={6} className="py-6 text-center text-white">
                                Nothing here.
                            </td>
                        </tr>
                    )}
            ...
            <div className="w-full h-fit px-4 py-3 rounded-lg border-satellite/20 flex flex-row justify-center gap-2 items-center mt-4">
                ...
                <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0} className="rounded-lg border border-satellite px-3 py-2 disabled:opacity-50">
                    Next
                </button>
            </div>
        </div>
